 ...
 * Toggle featured status of a contractor
 *
 * Request Body:
 * {
 *   featured?: boolean (optional - if not provided, toggles current state)
 * }
 *
 * Response:
 * {
...
export async function POST(
  ...
  try {
    ...
    const newFeatured = payload.featured !== undefined ? payload.featured : !installer.featured
    ...
    if (updateError) {
      console.error('Feature toggle error:', updateError)
      return NextResponse.json(
        { status: 'error', error: 'Failed to update featured status' },
        { status: 400 }
      )
    }
    ...
    console.error('POST /api/admin/contractors/:id/feature error:', error)
    return NextResponse.json(
